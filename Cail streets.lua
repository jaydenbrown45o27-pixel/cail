...
if shared._cali_cleanup then pcall(shared._cali_cleanup) end
...
local F = {
	aimbot = false, aimFov = 150, aimPart = "Head", aimTeamCheck = false, aimVisCheck = false,
	aimSmoothing = 0.25, aimKey = Enum.KeyCode.LeftShift, aimHoldOnly = false,
	esp = false, espBox = true, espHealth = true, espName = true, espDist = true, espSkeleton = true,
	espTeamCheck = false,
	espMaxDist = 2000,
	walkSpeed = false, walkSpeedVal = 30,
	noclip = false,
}

local function getChar() return LP.Character end
local function getHum(p) p = p or LP; local c = p.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP(p) p = p or LP; local c = p.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local function alive(p)
	local c = p.Character
	return c and c:FindFirstChildOfClass("Humanoid") and c.Humanoid.Health > 0 and c:FindFirstChild("HumanoidRootPart")
end
local function isTeammate(p)
	if not F.aimTeamCheck and not F.espTeamCheck then return false end
...
local function screenDistFromCenter(worldPos)
	...
	return (Vector2.new(sp.X, sp.Y) - Vector2.new(vs.X/2, vs.Y/2)).Magnitude
end
local function pickTarget()
	local best, bestScore
	for _, p in ipairs(Players_:GetPlayers()) do
		if p ~= LP and alive(p) then
			if F.aimTeamCheck and isTeammate(p) then
			else
				local char = p.Character
				local part = char:FindFirstChild(F.aimPart) or char:FindFirstChild("HumanoidRootPart")
				if part then
					if F.aimVisCheck and not visible(part) then
					else
						local sd = screenDistFromCenter(part.Position)
						if sd <= F.aimFov and (not bestScore or sd < bestScore) then
							best, bestScore = p, sd
						end
					end
				end
...
bind(UserInputService.InputBegan:Connect(function(input, gpe)
	...
	if input.KeyCode == F.aimKey then aimHeld = true end
end))
bind(UserInputService.InputEnded:Connect(function(input, gpe)
	if input.KeyCode == F.aimKey then aimHeld = false end
end))
bind(RunService.RenderStepped:Connect(function()
	if not F.aimbot then return end
	if F.aimHoldOnly and not aimHeld then return end
	local t = pickTarget()
	if not t then return end
	local char = t.Character
	local part = char and (char:FindFirstChild(F.aimPart) or char:FindFirstChild("HumanoidRootPart"))
	if not part then return end
	local target = CFrame.new(Camera.CFrame.Position, part.Position)
	Camera.CFrame = Camera.CFrame:Lerp(target, math.clamp(1 - F.aimSmoothing, 0.05, 1))
end))
...
local function makeDraw(kind, props)
...
local function newEsp(p)
	local d = { player = p }
	pcall(function()
		d.box = makeDraw("Square", { Thickness = 1, Color = Color3.fromRGB(255,255,255), Filled = false, Transparency = 1 })
		d.boxOutline = makeDraw("Square", { Thickness = 3, Color = Color3.fromRGB(0,0,0), Filled = false, Transparency = 1 })
		d.name = makeDraw("Text", { Size = 14, Color = Color3.fromRGB(255,255,255), Center = true, Outline = true, Font = 2 })
		d.dist = makeDraw("Text", { Size = 12, Color = Color3.fromRGB(200,200,200), Center = true, Outline = true, Font = 2 })
		d.hpBg = makeDraw("Square", { Thickness = 1, Color = Color3.fromRGB(0,0,0), Filled = true, Transparency = 1 })
		d.hp = makeDraw("Square", { Thickness = 1, Color = Color3.fromRGB(0,255,0), Filled = true, Transparency = 1 })
		d.hpTxt = makeDraw("Text", { Size = 11, Color = Color3.fromRGB(255,255,255), Center = false, Outline = true, Font = 2 })
		d.skele = {}
		for _, pair in ipairs(skeletonPairs) do
			table.insert(d.skele, makeDraw("Line", { Thickness = 1, Color = Color3.fromRGB(255,255,255), Transparency = 1 }))
		end
	end)
	d.highlight = Instance.new("Highlight")
	d.highlight.FillTransparency = 0.6
	d.highlight.OutlineTransparency = 0
	d.highlight.FillColor = Color3.fromRGB(255,80,80)
	d.highlight.OutlineColor = Color3.fromRGB(255,255,255)
	d.highlight.Enabled = false
	pcall(function() d.highlight.Parent = game:GetService("CoreGui") end)
	espData[p] = d
end
local function killEsp(d)
	...
	if d.hpTxt then pcall(function() d.hpTxt:Remove() end) end
	if d.skele then for _, l in ipairs(d.skele) do pcall(function() l:Remove() end) end end
	if d.highlight then pcall(function() d.highlight:Destroy() end) end
end
local function hideEsp(d)
	if not d then return end
	local list = { d.box, d.boxOutline, d.name, d.dist, d.hpBg, d.hp, d.hpTxt }
	for _, o in ipairs(list) do if o then o.Visible = false end end
	if d.skele then for _, l in ipairs(d.skele) do l.Visible = false end end
	if d.highlight then d.highlight.Enabled = false end
end

bind(Players_.PlayerAdded:Connect(function(p) newEsp(p) end))
bind(Players_.PlayerRemoving:Connect(function(p) killEsp(espData[p]); espData[p] = nil end))
for _, p in ipairs(Players_:GetPlayers()) do if p ~= LP then newEsp(p) end end

bind(RunService.RenderStepped:Connect(function()
	for _, p in ipairs(Players_:GetPlayers()) do
		if p ~= LP then
			local d = espData[p]
			if not d then newEsp(p); d = espData[p] end
			if not F.esp or not alive(p) then hideEsp(d) continue end
			if F.espTeamCheck and isTeammate(p) then hideEsp(d) continue end
			local char = p.Character
			local hrp = char.HumanoidRootPart
			local hum = char:FindFirstChildOfClass("Humanoid")
			local myHrp = getHRP()
			local dist = myHrp and (hrp.Position - myHrp.Position).Magnitude or 0
			if dist > F.espMaxDist then hideEsp(d) continue end
			d.highlight.Adornee = char
			d.highlight.Enabled = true
			local head = char:FindFirstChild("Head") or hrp
			local pos, onScreen = Camera:WorldToViewportPoint(hrp.Position)
			if not onScreen then hideEsp(d) continue end
			local topPos = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.7, 0))
			local botPos = Camera:WorldToViewportPoint(hrp.Position - Vector3.new(0, 3, 0))
			local h = math.abs(topPos.Y - botPos.Y)
			local w = h * 0.55
			local topLeft = Vector2.new(pos.X - w/2, topPos.Y)
			if F.espBox and d.box then
			...
			if F.espName and d.name then
				d.name.Position = Vector2.new(pos.X, topLeft.Y - 16)
				d.name.Text = p.Name
				d.name.Color = (p.Team and p.TeamColor.Color) or Color3.fromRGB(255,255,255)
				d.name.Visible = true
			elseif d.name then d.name.Visible = false end
			if F.espDist and d.dist then
				d.dist.Position = Vector2.new(pos.X, topLeft.Y + h + 2)
				d.dist.Text = string.format("[%dm]", math.floor(dist))
				d.dist.Visible = true
			elseif d.dist then d.dist.Visible = false end
			if F.espHealth and d.hpBg and hum then
				local hpFrac = math.clamp(hum.Health / math.max(hum.MaxHealth,1), 0, 1)
				d.hpBg.Position = Vector2.new(topLeft.X - 6, topLeft.Y)
				d.hpBg.Size = Vector2.new(3, h)
				d.hpBg.Visible = true
				d.hp.Position = Vector2.new(topLeft.X - 6, topLeft.Y + h * (1 - hpFrac))
				d.hp.Size = Vector2.new(3, h * hpFrac)
				d.hp.Color = Color3.fromRGB(math.floor(255 * (1-hpFrac)), math.floor(255 * hpFrac), 0)
				d.hp.Visible = true
				d.hpTxt.Position = Vector2.new(topLeft.X - 22, topLeft.Y)
			...
			if F.espSkeleton and d.skele then
				for i, pair in ipairs(skeletonPairs) do
					local a = char:FindFirstChild(pair[1])
					local b = char:FindFirstChild(pair[2])
					local line = d.skele[i]
			...
			elseif d.skele then
				for _, l in ipairs(d.skele) do l.Visible = false end
			end
		end
	end
end))
...
bind(RunService.Heartbeat:Connect(function(dt)
	if not F.walkSpeed then return end
	local hrp = getHRP(); local h = getHum()
	if not hrp or not h then return end
	local md = h.MoveDirection
	if md.Magnitude > 0 then
		local boost = F.walkSpeedVal - 16
		if boost > 0 then
			hrp.CFrame = hrp.CFrame + md * boost * dt
		end
	end
end))
...
local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Cali Streets" })

local CTab = Window:CreateTab("Combat")
local aim = CTab:CreateSection("Aimbot", "Left")
aim:AddToggle({ Text = "Aimbot", Default = false, Callback = function(v) F.aimbot = v end })
aim:AddSlider({ Text = "Aimbot FOV", Min = 20, Max = 800, Default = 150, Decimals = 0, Callback = function(v) F.aimFov = v end })
aim:AddDropdown({ Text = "Target Location", List = {"Head","HumanoidRootPart","UpperTorso"}, Default = "Head", Callback = function(v) F.aimPart = v end })
aim:AddSlider({ Text = "Aim Smoothing (0=snap)", Min = 0, Max = 0.9, Default = 0.25, Decimals = 2, Callback = function(v) F.aimSmoothing = v end })
aim:AddToggle({ Text = "Visibility Check", Default = false, Callback = function(v) F.aimVisCheck = v end })

local ETab = Window:CreateTab("ESP")
local eSec = ETab:CreateSection("Visuals", "Left")
eSec:AddToggle({ Text = "Player ESP", Default = false, Callback = function(v) F.esp = v end })
eSec:AddToggle({ Text = "Box ESP", Default = true, Callback = function(v) F.espBox = v end })
eSec:AddToggle({ Text = "Health ESP", Default = true, Callback = function(v) F.espHealth = v end })
eSec:AddToggle({ Text = "Name ESP", Default = true, Callback = function(v) F.espName = v end })
eSec:AddToggle({ Text = "Distance ESP", Default = true, Callback = function(v) F.espDist = v end })
eSec:AddToggle({ Text = "Skeleton ESP", Default = true, Callback = function(v) F.espSkeleton = v end })
eSec:AddSlider({ Text = "Max Distance", Min = 100, Max = 5000, Default = 2000, Decimals = 0, Callback = function(v) F.espMaxDist = v end })

local PTab = Window:CreateTab("Player")
local mv = PTab:CreateSection("Movement", "Left")
mv:AddToggle({ Text = "Cframe speed", Default = false, Callback = function(v) F.walkSpeed = v end })
mv:AddSlider({ Text = "Speed Value", Min = 16, Max = 100, Default = 30, Decimals = 0, Callback = function(v) F.walkSpeedVal = v end })
mv:AddToggle({ Text = "Noclip", Default = false, Callback = function(v) F.noclip = v end })

shared._cali_cleanup = function()
	ALIVE = false
	for _, c in ipairs(conns) do pcall(function() c:Disconnect() end) end
	for _, d in pairs(espData) do killEsp(d) end
